...
@frappe.whitelist()
def get_coa_table_data(company):
	"""Get Chart of Accounts data in table format"""
	...
	# Calculate levels and get balances
	for account in accounts:
		# Calculate hierarchy level
		account['level'] = frappe.db.count('Account', {
			'lft': ['<', account['lft']],
			'rgt': ['>', account['rgt']],
			'company': company
		})
		
		# Get account balance if show_balance_in_coa is enabled
		if frappe.db.get_single_value("Accounts Settings", "show_balance_in_coa"):
			try:
				balance = get_balance_on(account['name'], company=company)
				account['balance'] = balance
				account['company_currency'] = frappe.get_cached_value("Company", company, "default_currency")
			except:
				account['balance'] = 0
				account['company_currency'] = frappe.get_cached_value("Company", company, "default_currency")
		else:
