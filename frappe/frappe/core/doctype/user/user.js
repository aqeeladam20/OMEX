frappe.ui.form.on("User", {
	...
	refresh: function (frm) {
		...
		frappe.xcall("frappe.apps.get_apps").then((r) => {
			...
			if (r) {
				r.forEach((app) => {
					app_titles[app.name] = app.title;
				});
			}
			frm.set_df_property("default_app", "options", [" ", ...apps]);
			
			// Store app titles for display
			frm.app_titles = app_titles;
			
			// Override the display of the select field
			setTimeout(() => {
				let field = frm.fields_dict.default_app;
				if (field && field.$input) {
					field.$input.find('option').each(function() {
						let value = $(this).val();
						if (value && app_titles[value]) {
							$(this).text(app_titles[value]);
						}
					});
				}
			}, 100);
