...
export default class WidgetGroup {
	...
	make_container() {
		const widget_area = $(`<div class="widget-group ${this.class_name || ""}">
				...
				<div class="widget-group-body widget-flow-container" data-columns="${this.columns}">
				</div>
			</div>`);
	...
	add_widget(widget) {
		// Create individual widget wrapper for better flow
		const widget_wrapper = $(`<div class="widget-wrapper" data-widget-name="${widget.name}"></div>`);
		this.body.append(widget_wrapper);

		let widget_object = frappe.widget.make_widget({
			...widget,
			widget_type: this.type,
			container: widget_wrapper,
