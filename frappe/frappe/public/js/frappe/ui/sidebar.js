frappe.ui.Sidebar = class Sidebar {
	...
	make_dom() {
		...
		// Add responsive click behavior
		this.setup_responsive_sidebar();
	...
	setup_responsive_sidebar() {
		const $body_sidebar = this.wrapper.find(".body-sidebar");
		const $main_section = $(".main-section");
		const $overlay = this.wrapper.find(".overlay");
		
		// Handle clicks on sidebar items and icons
		$body_sidebar.on("click", (e) => {
			// If sidebar is collapsed, expand it on any click
			if (!this.sidebar_expanded) {
				// Check if it's a link click - if so, let it navigate after expanding
				const $target = $(e.target);
				const $link = $target.closest('a.item-anchor');
				
				if ($link.length > 0) {
					// It's a link click - expand sidebar and allow navigation
					this.open_sidebar();
					// Don't prevent default - let the link navigate
					return;
				} else {
					// It's not a link - just expand sidebar
					e.preventDefault();
					e.stopPropagation();
					this.open_sidebar();
					return false;
		...
		// Click on main content to collapse when expanded
		$main_section.on("click", (e) => {
			if (this.sidebar_expanded && !$(e.target).closest(".body-sidebar-container").length) {
				this.close_sidebar();
			}
		});
		...
		// Prevent clicks inside expanded sidebar from closing it (only when expanded)
		$body_sidebar.on("click", ".sidebar-items, .sidebar-item-container, .standard-sidebar-item", (e) => {
			if (this.sidebar_expanded) {
				...
				if ($link.length === 0) {
					// Not a link click - prevent closing
					e.stopPropagation();
				}
				// If it's a link click, let it bubble up normally for navigation
			}
		});
		
		// Handle escape key to close sidebar
		$(document).on("keydown", (e) => {
			if (e.key === "Escape" && this.sidebar_expanded) {
				this.close_sidebar();
			}
		});
	}
